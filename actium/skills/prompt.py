"""生成 <available_skills> XML prompt block"""

import html
from pathlib import Path
from actium.skills.parser import find_skill_md, read_properties


def to_prompt(skill_dirs: list[Path]) -> str:
    """生成 <available_skills> XML block 用于 agent system prompts
    
    这个 XML 格式是 Anthropic 推荐用于 Claude 模型的格式。
    Skill Clients 可以根据自己的模型或偏好格式化 skill 信息。
    
    Args:
        skill_dirs: Skill 目录路径列表
        
    Returns:
        包含每个 skill 的 name, description, location 的 XML 字符串
        
    Example output:
        <available_skills>
        <skill>
        <name>pdf-reader</name>
        <description>Read and extract text from PDF files</description>
        <location>/path/to/pdf-reader/SKILL.md</location>
        </skill>
        </available_skills>
    """
    if not skill_dirs:
        return "<available_skills>\n</available_skills>"
    
    lines = ["<available_skills>"]
    
    for skill_dir in skill_dirs:
        skill_dir = Path(skill_dir).resolve()
        
        try:
            props = read_properties(skill_dir)
        except (ValueError, FileNotFoundError):
            # 跳过不符合规范的 skill
            continue
        
        skill_md_path = find_skill_md(skill_dir)
        if skill_md_path is None:
            continue
        
        lines.append("<skill>")
        lines.append("<name>")
        lines.append(html.escape(props.name))
        lines.append("</name>")
        lines.append("<description>")
        lines.append(html.escape(props.description))
        lines.append("</description>")
        lines.append("<location>")
        lines.append(str(skill_md_path))
        lines.append("</location>")
        lines.append("</skill>")
    
    lines.append("</available_skills>")
    
    return "\n".join(lines)

