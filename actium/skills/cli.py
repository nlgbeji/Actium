...
def init(name: str, skills_dir: str, description: Optional[str], license: Optional[str], compatibility: Optional[str]):
    """
    初始化一个新的 skill 目录
    ...
    if not validator.validate_name(name):
        ...
        click.echo("名称要求: 1-64字符，小写字母、数字、连字符，不能以连字符开头/结尾")
        return
    ...
    skill_md_content = SKILL_MD_TEMPLATE.format(
        frontmatter=yaml.dump(frontmatter, allow_unicode=True, default_flow_style=False).strip(),
        body=f"# {name}\n\n在此添加 skill 的使用说明。\n"
    )
...
@click.option('--skills-dir', '-s', required=True, help='Skills 目录路径')
@click.option('--output', '-o', help='输出目录（默认: skill/docs/）')
@click.option('--format', '-f', type=click.Choice(['md', 'json']), default='md', help='文档格式')
def build_doc(skill_name: str, skills_dir: str, output: Optional[str], format: str):
    ...
    output_dir = Path(output) if output else skill_path / "docs"
