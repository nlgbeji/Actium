...
def create_scatter_plot(
    ...
    scatter_kwargs: Dict[str, Any] = {
        'alpha': alpha,
        's': 50 if size is None else size
    }
...
def create_line_plot(
    ...
    if multiple_series is not None:
        for i, series in enumerate(multiple_series):
            ...
            ax.plot(x, series, linewidth=linewidth, label=label, marker='o', markersize=4)
        if labels:
...
def create_histogram(
    ...
    if kde:
        ...
        ax.hist(data, bins=bins, color=color, alpha=alpha, edgecolor='black', linewidth=1.2)
